...
class CNN_LSTM(nn.Module):
    def __init__(self, arch, num_classes, lstm_layers, hidden_size):
        ...
        if arch.startswith('resnet18'):
            ...
            self.cnn[0] = nn.Conv2d(2, 64, kernel_size=7, stride=2, padding=3, bias=False)
...
class WindModelE2E(nn.Module):
    ...
    def arflow(self, tensorFirst, tensorSecond): #b, 3, h ,w
        ...
        return flow_to_image, flows_1221
    ...
    def forward(self, inputs): #inputs will be a batch of 10 frames of a video, 10*3*H*W
        ...
        w = inputs.size(3)
        h = inputs.size(4)
        ofs = torch.zeros(batch_size, length-1, 2,w,h).cuda()
        flows_1221_frame =[]
...
if __name__ == '__main__':
    ...
    macs, params = get_model_complexity_info(model, (10, 3, 256, 256), as_strings=True,
                                           print_per_layer_stat=True, verbose=True)
    print('{:<30}  {:<8}'.format('Computational complexity: ', macs))
    print('{:<30}  {:<8}'.format('Number of parameters: ', params))
    # stat(model, (1,10,3, 256, 256))
