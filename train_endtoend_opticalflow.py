...
from Model_optical_opticalflow import WindModelE2E
...
parser.add_argument('--model', default='./save_model_focal_unsperoptical_opticalflow', type=str, help = 'path to model')
...
def save_checkpoint(state, is_best, epoch,  best_prec, val_prec, filename='.pth.tar'):
    torch.save(state, os.path.join('./save_model_focal_unsperoptical_opticalflow/', str(epoch) + '_test' + str(best_prec) + '_val' + str(val_prec) + filename),_use_new_zipfile_serialization=False)
