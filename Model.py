...
class SpatialCNN(nn.Module):
    def __init__(self, arch, num_classes):
        ...
        if arch.startswith('resnet18'):
            self.cnn = resnet18(pretrained=True, channel=3, num_classes=num_classes).cuda()
...
class MotionCNN(nn.Module):
    def __init__(self, arch, channels, num_classes):
        ...
        if arch.startswith('resnet18'):
            self.cnn = resnet18(pretrained=True, channel=channels, num_classes=num_classes).cuda()
...
class CNN_LSTM(nn.Module):
    def __init__(self, arch, num_classes, lstm_layers, hidden_size):
        ...
        self.fc_pre = nn.Sequential(nn.Linear(512, 256), nn.Dropout())
        self.rnn = nn.LSTM(input_size=256, hidden_size=self.hidden_size, num_layers=self.lstm_layers, batch_first=True)
    ...
    def forward(self, inputs):
        ...
        # print(out[:,-1,:])
        x = self.dropout(self.sigmoid(self.fc1(out[:,-1,:])))
        x = self.fc2(x)
        return x
...
class WindModelE2E(nn.Module):
    def __init__(self,arch, num_classes, lstm_layers, hidden_size):
        ...
        self.lstm_layers = lstm_layers
        self.epsilon = 1e-4
        self.OpticalNet = PWCNetwork().cuda()
    ...
    def arflow(self, tensorFirst, tensorSecond): #b, 3, h ,w
        ...
        print(flows_1221.shape)

        flow_to_image = res_dict['flows_fw'][0]

        print(flow_to_image.shape)
    ...
    def forward(self, inputs): #inputs will be a batch of 10 frames of a video, 10*3*H*W
        ...
        flows_1221_frame = torch.zeros(batch_size, length-1, 4,w,h).cuda()
        for i in range(length-1):
            ...
            flows_1221_frame[:,i,:,:,:] = torch.FloatTensor(flows_1221)
        ...
        return outputs_fusion, flows_1221_frame
...
if __name__ == '__main__':
    ...
    inputs = torch.randn(16, 10, 3, 224, 224).cuda()
    output = model(inputs)
    print(output.shape)
