...
from Model import WindModelE2E
...
parser.add_argument('--model', default='./save_model_focal', type=str, help = 'path to model')
...
def save_checkpoint(state, is_best, epoch,  best_prec, val_prec, filename='.pth.tar'):
    torch.save(state, os.path.join('./save_model_mixup/', str(epoch) + '_test' + str(best_prec) + '_val' + str(val_prec) + filename))
...
def train(train_loader, model, criterion, criterion1, optimizer, epoch):
    ...
    for i, (inputs, target, _) in enumerate(train_loader):
        ...
        inputs, targets_a, targets_b, lam = mixup_data(input_var, target_var)
        inputs, targets_a, targets_b = map(Variable, (inputs,
                                                      targets_a, targets_b))
        targets_a_onehot = torch.nn.functional.one_hot(targets_a, 11)
        targets_b_onehot = torch.nn.functional.one_hot(targets_b, 11)
        # Check input
        ...
        output, frame_1221_frame = torch.where(torch.isnan(output), torch.full_like(output, 0), output)

        # output_check =output.cpu().detach().numpy()
        
        # print(np.any(np.isnan(output_check)))

        loss = mixup_criterion(criterion, output, targets_a, targets_b, lam) + mixup_criterion(criterion1, output, targets_a_onehot, targets_b_onehot, lam)
...
if __name__ == '__main__':
    ...
    data_dir_train = '/home/bigspace/xujialang/Wind_Project/VisualWind_split/train'
    data_dir_val = '/home/bigspace/xujialang/Wind_Project/VisualWind_split/val'
    data_dir_test = '/home/bigspace/xujialang/Wind_Project/VisualWind_split/test'
    ...
    train_dataset = VideoDataset(data_dir_train, transform[0], flow=False)
    val_dataset = VideoDataset(data_dir_train, transform[1], flow=False)
    test_dataset = VideoDataset(data_dir_train, transform[1], flow=False)
    ...
    if os.path.exists(os.path.join(args.model, 'checkpoint.pth.tar')):
        ...
        model.OpticalNet.load_state_dict(model_info['state_dict1'])
    ...
    for epoch in range(cur_epoch, args.epochs):
        ...
        # train on one epoch
        train_loss, train_top1 = train(train_loader, model, criterion, criterion1, optimizer, epoch)
        ...
        if prec1 > best_prec:
            ...
            save_checkpoint({
                ...
                'state_dict1': model.OpticalNet.state_dict(),
                'state_dict2': model.Classifier.state_dict(),
                'state_dict3': model.Classifier_rgb.state_dict(),
                'best_prec': best_prec,
