...
from Model_optical_nooptical import WindModelE2E
...
parser.add_argument('--model', default='./save_model_focal_unsperoptical_nooptical', type=str, help = 'path to model')
...
def save_checkpoint(state, is_best, epoch,  best_prec, val_prec, filename='.pth.tar'):
    torch.save(state, os.path.join('./save_model_focal_unsperoptical_nooptical/', str(epoch) + '_test' + str(best_prec) + '_val' + str(val_prec) + filename))
...
def train(train_loader, model, criterion, criterion1, optical_loss_func, optimizer, epoch):
    ...
    for i, (inputs, target, _) in enumerate(train_loader):
        ...
        # print('optical_loss: ', optical_loss)
        # print('l_ph: ', l_ph)
        # print('l_sm: ', l_sm)

        loss = criterion(output, target_var) + criterion1(output, target_var_onehot)
...
if __name__ == '__main__':
    ...
    for epoch in range(cur_epoch, args.epochs):
        ...
        # train on one epoch
        train_loss, train_top1 = train(train_loader, model, criterion, criterion1, criterion1, optimizer, epoch)
        ...
        if prec1 > best_prec:
            ...
            save_checkpoint({
                ...
                # 'state_dict1': model.ARFlow.state_dict(),
